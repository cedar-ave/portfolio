...
import {
  ...
  SafeWrites,
  ReviewStack,
} from '@site/src/components/SkillDiagrams';

# AI agent skills

I design, build, and maintain a suite of Claude AI agent skills that cover the full lifecycle of end-user help documentation: gathering source knowledge, generating articles, reviewing them for style, terminology, and AI readiness, publishing through Paligo XML CCMS and Zendesk, and writing release notes. The skills ship together as a versioned Claude Code plugin with custom MCP servers, so teammates can install and update the whole toolkit in one step.
...
## Source research AI agent skills
...
## Content management automation AI agent skills
...
## Content quality optimization AI agent skills
...
**What it is**: An audit skill that checks a Google Doc, Markdown file, text file, or Word document against an organization's custom controlled vocabulary. It flags every non-preferred term and gives the preferred term, the style guide's reasoning, and a suggested rewrite.
...
**What it is**: An audit skill that checks a document against an organization's custom Technical Style Guide, with the Microsoft Style Guide as a fallback. It returns a numbered list of violations with a suggested fix for each, and I approve fixes one at a time.
